...
Implementation of FHIR Terminology Service specification (https://www.hl7.org/fhir/R4/terminology-service.html) to expose arbitrary persistent classes as FHIR value sets and code systems. Runs on InterSystems IRIS for Health 2020.1-2021.1.
...
4. Create InteractionsStrategy class for your FHIR Terminology Service endpoint, **or skip this step and use [Sample.iscru.fhir.fts.SimpleStrategy](../main/samples/cls/Sample/iscru/fhir/fts/SimpleStrategy.cls)**. If you choose to create a new InteractionsStrategy class, then do the following:
   * subclass [iscru.fhir.fts.FTSStrategy](../main/src/cls/iscru/fhir/fts/FTSStrategy.cls),
   * override ```StrategyKey``` class parameter,
...
Create RepoManager class for your FHIR Terminology Service endpoint (**or skip this step if you are using [Sample.iscru.fhir.fts.SimpleStrategy](../main/samples/cls/Sample/iscru/fhir/fts/SimpleStrategy.cls)**):
   * subclass ```HS.FHIRServer.Storage.Json.RepoManager``` system class,
   * override ```StrategyClass``` and ```StrategyKey``` class parameters as follows:
```
...
6(a). _This step is only applicable to 2020.1-2020.3 versions of InterSystems IRIS._

Create ```terminology``` directory within ```<installation directory>/dev/fhir/fhir-metadata``` and copy [dummy-search-parameters.json](../main/src/fhir-search-parameters/dummy-search-parameters.json) file there.
...
Create a custom FHIR metadata set based on R4 set with additional search parameters defined in ```<installation directory>/dev/fhir/fhir-metadata/terminology/dummy-search-parameters.json```.
...
...
```
6(b). _This step is only applicable to 2020.4 or newer versions of InterSystems IRIS._

Import FHIR metadata package from [package](../main/src/fhir-search-parameters/package) directory either [using the Management Portal](https://docs.intersystems.com/irisforhealthlatest/csp/docbook/Doc.View.cls?KEY=HXFHIR_server_customize#HXFHIR_server_customize_packages_import) or the interactive utility:
```
TERMINOLOGY>do ##class(HS.FHIRServer.ConsoleSetup).Setup()
What do you want to do?
  0)  Quit
...
7. Create a new FHIR endpoint based on your custom InteractionsStrategy class (or on [Sample.iscru.fhir.fts.SimpleStrategy](../main/samples/cls/Sample/iscru/fhir/fts/SimpleStrategy.cls)) and on the just imported metadata package (or on the new metadata set - for IRIS versions prior to 2020.4) either [using the Management Portal](https://docs.intersystems.com/irisforhealthlatest/csp/docbook/Doc.View.cls?KEY=HXFHIR_server_install) or the interactive utility:
```
TERMINOLOGY>do ##class(HS.FHIRServer.ConsoleSetup).Setup()
What do you want to do?
  0)  Quit
